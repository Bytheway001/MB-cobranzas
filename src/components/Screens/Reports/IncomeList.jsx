...
export const IncomeList = ({ incomes, setCorrection }) => {
	...
	const columns = [
		...
		},
		{
			...
			sort: true,
			filter: selectFilter({
				options: listOf(incomes, "account.name"),
		...
		{ dataField: "description", text: "Desc." },
		{
			...
			text: "Categoria",
			filter: selectFilter({ options: categories, className: "form-control form-control-sm", placeholder: "Todas" }),
		},
				...
				<>
					{!row.corrected_with && (
						<ButtonGroup size="sm">
							...
							<Button
								size="sm"
								onClick={() => setCorrection({ type: "incomes", row: row, user_id: row.user_id, user: row.user })}
							>
	...
	return (
		<ToolkitProvider
			keyField="id"
			data={incomes.filter((x) => x.category.name !== "Correcciones" && !x.corrected_with && !x.correcting)}
			columns={columns}
			exportCSV={{
