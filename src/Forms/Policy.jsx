...
const PolicyForm = ({ companies, getCompanies,  policy, client, createPolicy }) => {
    ...
    const onFormSubmit = (d) => {
        ...
        createPolicy(policy).then(res=>{
            addNotification('success',policy.id?"Poliza Actualizada":"Poliza Creada")
            handleClose();
        })
    ...
    return (
        <>
            <Button block size='sm' variant="primary" onClick={handleShow}>{!policy ? "Agregar Poliza" : "Editar Poliza"}</Button>
            <Modal size='lg' show={show} onHide={handleClose}>
                ...
                <Modal.Body>
                    <Form
                        ...
                        initialValues={{ ...policy, client_id: client }}
                    >
                        {({ handleSubmit, values }) => (
                            <form id='policy-form' onSubmit={handleSubmit}>
                                ...
                                <Row>
                                                    ...
                                                    return (
                                                        <>
                                                            ...
                                                            <Typeahead
                                                                ...
                                                                isValid={!meta.error && meta.dirty} id='company-selector'
                                                                onChange={(val) => input.onChange(val[0])}
                                                                selected={input.value ? Array.isArray(input.value) ? input.value : [companies.find(x => x.id === parseInt(input.value.id))] : []}
                                                                name={input.name} size='sm'
                                                                labelKey='name' options={companies}
                                                            />
                                                            {meta.dirty && meta.error && <span className='error-feedback'>{meta.error}</span>}
                                                        </>
                                                    )
                                                }
                                                }
                                                ...
                                                {({ input, meta }) => {
                                                    return (
                                                        <>
                                                            ...
                                                            <label>Plan</label>
                                                            <Typeahead isInvalid={meta.error ? true : false}
                                                                isValid={!meta.error && meta.dirty} id='plan-selector'
                                                                onChange={(val) => {
                                                                    input.onChange({
                                                                        ...val[0],
                                                                ...
                                                                name={input.name} size='sm'
                                                                labelKey='name' options={values.company ? companies.find(x => x.id === values.company.id).plans : []}
                                                            />
                                                            {meta.dirty && meta.error && <span className='error-feedback'>{meta.error}</span>}
                                                        </>
                                                    )
                                            ...
                                            <Field name='frequency' validate={composeValidators(Validators.required)} >
                                                {({ input, meta }) => (
                                                    <>
                                                        ...
                                                        <FormControl as='select' isInvalid={meta.error && meta.touched} isValid={!meta.error && meta.touched} size='sm' {...input}>
                                    ...
                                    <Col sm={6}>
                                                ...
                                                {({ input, meta }) => (
                                                    <>
                                                        ...
                                                        <ReactDatePicker
                                                            ...
                                                            selected={input.value ? new Date(input.value.replace(/(\d{2})\/(\d{2})\/(\d{4})/, "$2/$1/$3")) :""}
                                        ...
                                        <FormGroup>
                                            <Field name='premium' validate={composeValidators(Validators.required, Validators.mustBeDecimal)} >
                                                {({ input, meta }) => (
                                                    <>
                                            ...
                                            <Field name='comment'  >
                                                {({ input, meta }) => (
                                                    <>
                                                        ...
                                                        <FormControl as='textarea' isInvalid={meta.error && meta.touched} isValid={!meta.error && meta.touched} size='sm' {...input} />
                                                        {meta.error && meta.touched && <span className='error-feedback'>{meta.error}</span>}
                                                    </>
                ...
                <Modal.Footer>
                    ...
                    <Button size='sm' type='submit' form='policy-form' variant="primary" >{!policy ? "Crear Poliza" : "Guardar Cambios"}</Button>
                </Modal.Footer>
            </Modal>
        </>
    )
