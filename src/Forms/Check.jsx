...
const CheckForm = () => {
    ...
    const onCheckFormSubmit = (values) => {
        Axios.post(API + '/checks/collect', values).then(res => {
            alert(res.data.data)
        })
    }
    return (
        <>
            ...
            <Modal size='md' show={show} onHide={() => setShow(false)}>
                            ...
                            <form onSubmit={handleSubmit} id='check-form'>
                                <FormGroup>
                                    ...
                                    <Field name='check_id' validate={Validators.required}>
                                        {({ meta, input }) => (
                                            <FinalFormSelect fieldProps={{ meta, input }} options={checks.map((check, k) => <option key={k} value={check.id}>{
                                                `${check.client.first_name} - ${formatMoney(check.amount, 2, '.', ',', ' ' + check.currency)}`
                                            }</option>)} />
                                        )}
                                    </Field>
                                </FormGroup>
                                <FormGroup>
                                    ...
                                    <Field name='account_id' validate={composeValidators(
                                        Validators.required,
                                        (value) => Validators.mustNotBe(value, ['101', '100', '109', '9'])
                                    )}>
                                        {({ meta, input }) => (
                ...
                <Modal.Footer>
                    ...
                    <Button type='submit' form='check-form'>Cobrar Cheque</Button>
                </Modal.Footer>
            </Modal>
        </>
    )
